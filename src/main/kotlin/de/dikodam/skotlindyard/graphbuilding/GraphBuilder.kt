...
fun buildGraph(file: File): Graph {
    val edgeInfo = readLinesFromFile(file)
        .map { extractEdgeInfoFromLine(it) }
...
fun extractVertices(edgeInfos: Collection<EdgeInfo>): Map<Int, Vertex> {
...
fun buildEdges(edgeInfos: Collection<EdgeInfo>, vertices: Map<Int, Vertex>): List<Edge> =
